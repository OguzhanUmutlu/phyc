...
namespace PhyC {
    ...
    struct RigidBody {
        ...
        std::vector<SurfaceTriangleElement> surface{}; // Defines mass and aerodynamic surfaces, are rendered
        ...
        void computeForcesAt(FTState& state, double dt) const;
        void computeGravityForce(FTState& state) const;
        void computeAeroSurfaceForces(FTState& state, const AirProperties& air) const;
        void computeAeroDampingTorque(FTState& state, const AirProperties& air) const;
